---
description: Reload — re-executing a request in place, preserving the wrapper object identity across sync and async modes.
---

# Reload

`reload` refreshes an already-held response by re-executing its original request,
without losing the object reference. It is aimed at consumers who keep a
`Response`/`AsyncResponse` reference and need to re-run the exact same request later.
