...
The mode (sync/async) and engine are selected by the `adapter` argument. The response
wrappers (`Response` / `AsyncResponse`) and the polling routine (`poll`) are **not**
re-exported here — reach them through the `resq.http` submodule only when needed.
